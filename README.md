...
**DeckView — от кода колоды до готового изображения.** Разработал сервис генерации изображений для Telegram и веб-приложений: персонализация оформления, фоновые Redis/RQ workers, многоуровневое кеширование и дедупликация запросов. Один backend обслуживает бота и HTTP API.

[Разбор проекта: рендер изображений, очереди, кеширование и выпуск →](https://github.com/Zulut30/Zulut30/blob/main/case-studies/deckview.md)
...
- **Очереди и производительность.** Выношу тяжёлую обработку в отдельные Redis/RQ workers, объединяю одинаковые запросы и использую многоуровневое кеширование с учётом версий данных.
- **Генерация изображений.** Разрабатываю конвейеры рендера с персонализацией оформления и проверками визуальных регрессий. Один backend обслуживает Telegram и HTTP API.
...
- **Деплой и инфраструктура.** Разворачиваю сервисы, подключаю домены, настраиваю DNS и Nginx. Реализую атомарное переключение релизов, health-check и откат при сбое.
...
| **[DeckView](https://github.com/Manacost-Labs/Deckview-TG)** | Генерация изображений для Telegram и веб-приложений: персонализация, Redis/RQ workers, версионируемый кеш и дедупликация запросов. Python, Pillow, Flask. |
